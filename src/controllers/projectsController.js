...
export const addProject = async (req, res) => {
    try {
        const { user_id, client_id, name, type, description, status, start_date, end_date } = req.body;
        ...
        if (status && !['active','inactive','archived'].includes(status)) return res.status(400).json({ error: "Invalid status" });
        ...
        const inserted = await sql`
            INSERT INTO projects (
                id, user_id, client_id, name, type, description, status, start_date, end_date, created_at, updated_at
            ) VALUES (
                ${id}, ${user_id}, ${client_id || null}, ${name}, ${type || 'private'}, ${description || null}, ${status || 'active'}, ${start_date || null}, ${end_date || null}, NOW(), NOW()
            ) RETURNING *
        `;
...
export const listProjects = async (req, res) => {
    try {
        ...
        if (status) { const allowed = ['active','inactive','archived']; if (!allowed.includes(status)) return res.status(400).json({ error: "Invalid status" }); filters.push(`status = $${idx++}`); params.push(status); }
