...
export async function initializeDatabase() {
    ...
    `;
        ...
        // Ensure friendly_name column exists
        try { await sql`ALTER TABLE newsletter ADD COLUMN IF NOT EXISTS friendly_name TEXT DEFAULT ''`; } catch (e) {}

        // Create indexes for newsletter
        try { await sql`CREATE INDEX IF NOT EXISTS idx_newsletter_email ON newsletter(email)`; } catch (e) {}
        try { await sql`CREATE INDEX IF NOT EXISTS idx_newsletter_active ON newsletter(active)`; } catch (e) {}
        try { await sql`CREATE INDEX IF NOT EXISTS idx_newsletter_created_at ON newsletter(created_at DESC)`; } catch (e) {}
        ...
        try {
            ...
            await sql`
                DROP TRIGGER IF EXISTS newsletter_updated_at_trigger ON newsletter
            `;
